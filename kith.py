...
from spam_banner import LIKELY_SPAMMER_USER_ID, make_likely_spammer_banner_file
...
# =========================
@bot.command(name="testspambanner")
async def test_spam_banner(ctx: commands.Context):
    if BOT_OWNER_ID == 0:
        await ctx.send("`BOT_OWNER_ID` is not set in .env.")
        return
    ...
    await ctx.send(file=make_likely_spammer_banner_file())
...
async def on_message(message: discord.Message):
    ...
    if message.author.id == LIKELY_SPAMMER_USER_ID:
        try:
            await message.channel.send(file=make_likely_spammer_banner_file())
